...
class Message(BaseModel):
    """消息模型"""
    role: str = Field(..., description="消息角色，包括 'user' 或 'assistant'")
    content: str = Field(..., description="消息内容")

class StartSessionRequest(BaseModel):
    """开始会话的请求模型"""
    user_id: str   # 必需
    doctor_info: Optional[str] = None # 医生信息（可选）
...
class StudentQueryRequest(BaseModel):
    ...
    context: str = Field(...,description="学生当前状态")# 必需额外上下文

class SessionResponse(BaseModel):
    """会话响应结构"""
    session_id: str  # 会话ID
    response: str = Field(...,description="助手的第一条响应/欢迎消息")
    messages: List[Message] = Field(..., description="包含欢迎消息的对话列表")
...
async def start_dialog_session(request: StartSessionRequest):
    """创建一个新的对话会话，支持个性化医生信息"""
    try:
        # 创建新会话
        ...
        if request.doctor_info:
            ...
            welcome_message = await loop.run_in_executor(
                process_pool,
                app.state.system.generate_welcome_message,
                request.doctor_info
            )
        else:
            # 默认欢迎消息
        ...
        # 构建响应消息列表
        messages = [Message(role="assistant", content=welcome_message)]
...
async def dialog_query(request: DialogQueryRequest):
    ...
    try:
        ...
        for msg in request.messages:
            session_data.add_message(msg.role, msg.content)
        ...
        logger.info(f"处理用户查询 [session={session_data.session_id}]: {current_question[:50]}...")
        ...
        async def response_generator():
            ...
            try:
                ...
                gen = await loop.run_in_executor(
                    process_pool,
                    app.state.system.stream_response_with_history,  # 使用支持历史的方法
                    messages_dict
                )
...
#     """获取指定会话的历史记录"""
#     logger.info(f"正在根据会话id（{session_id}）获取会话历史")
...
async def student_query(request: StudentQueryRequest):
    """根据学生当前状态（必需）以及家长目前关注的问题（可选）得到沟通建议并返回完整响应"""
    request_id = f"req_{uuid.uuid4().hex[:8]}"
    try:
        logger.info(f"正在根据学生状态获取沟通建议 [student={request.student_id}, request={request_id}]: {request.context[:50]}...")
        
        # 构建完整问题（包含学生ID）
        full_question = f"学生ID: {request.student_id}\n问题: {request.context}"
        if request.question:
            full_question += f"\n额外上下文(重点关注): {request.question}"
        ...
        logger.info(f"学生状态与建议已完成 [student={request.student_id}, request={request_id}]")
        ...
        logger.error(f"学生状态与建议处理失败: {e}", exc_info=True)
        raise HTTPException(
            status_code=500, 
            detail=f"处理学生状态与建议时发生错误: {str(e)}"
        )
